 ...
 */
@RunWith(Arquillian.class) public class ListAddRemoveEntryTestCase extends WiseTest {
    ...
    @Test public void listOfStringTest() {
        // page: step 1
    ...
    private void checkStepTwoData(List<String> inputList) {
        ...
        try {
            ...
            WebElement addButton = browser.findElement(By.className(PropUtils.get("tag.wise-gwt-button-add")));
            Assert.assertNotNull("An 'Add' button is expected but not found for URL: " + browser.getCurrentUrl(), addButton);
            ...
            List<WebElement> webElementList = browser.findElements(By.className(PropUtils.get("tag.wise-input-row")));
            Assert.assertTrue("Expected to find " + inputList.size() + " input items but found " + webElementList.size(),
                    inputList.size() == webElementList.size());

            // set input data
            for (int i = 0; i < inputList.size(); i++) {
                WebElement we = webElementList.get(i);
                WebElement inputBox = we.findElement(By.className(PropUtils.get("tag.wise-gwt-inputBox")));
                ...
                WebElement checkbox = we.findElement(By.className(PropUtils.get("tag.gwt-SimpleCheckBox")));
                Assert.assertNotNull("Did not find checkbox for item number, " + i, checkbox);
                Assert.assertTrue("Expected checkbox for item number " + i + " to be select but is not", checkbox.isSelected());
            }

            // confirm remove button works
            List<WebElement> removeButtonList = browser.findElements(By.className(PropUtils.get("tag.wise-gwt-button-remove")));
            Assert.assertNotNull("Did not find remove button for item number ", removeButtonList);

            for (int i = 0; i < removeButtonList.size(); i++) {
                WebElement removeButton = browser.findElement(By.className(PropUtils.get("tag.wise-gwt-button-remove")));
                Assert.assertNotNull("Did not find remove button for item number, " + i, removeButton);
                removeButton.click();
            }

            // result list
            List<WebElement> resultList = browser.findElements(By.className(PropUtils.get("tag.wise-input-row")));
            Assert.assertFalse("Expected to find 0 input items but found " + resultList.size(), resultList.isEmpty());
