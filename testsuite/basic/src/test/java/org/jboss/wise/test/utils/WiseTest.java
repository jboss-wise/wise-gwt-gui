...
public abstract class WiseTest {
    ...
    protected void loadStepOneOfThree(String homepageInputUrl) {
        ...
        try {
            ...
            Assert.fail("Setup ERROR: Unable to find input box for URL: inputBoxLabel: " + PropUtils.get("homepage.input.box"));
        }
        ...
        try {
            ...
            Assert.fail("Setup ERROR: Failed to retrieve WSDL: " + homepageInputUrl);
        }
    }
    ...
    protected void loadStepTwoOfThree() {
        ...
        WebElement nextButton = browser.findElement(By.className(PropUtils.get("tag.wise-gwt-Button-next")));
        Assert.assertTrue("Setup ERROR: Next button should be enabled but is not.", nextButton.isEnabled());
    ...
    protected void loadStepThreeOfThree() {
        ...
        try {
            ...
            Assert.assertNotNull("Setup ERROR: TextBox was expected but was not found.", inputBox);
    ...
    protected void checkStepOneData(String key, String propKey) {
        ...
        // confirm the next button is enabled based upon tree item selection
        WebElement nextButton = browser.findElement(By.className(PropUtils.get("tag.wise-gwt-Button-next")));
        Assert.assertTrue("Next button should be enabled but is not.", nextButton.isEnabled());

        nextButton.click();
        Graphene.waitModel().withTimeout(30, TimeUnit.SECONDS).until().element(By.className(propKey)).is().present();
    }
    ...
    protected void checkStepTwoData(String inputValue, boolean isCheckboxPresent) {
        ...
        try {
            ...
            // set the input
            try {
                WebElement inputBox = browser.findElement(By.className(PropUtils.get("tag.wise-gwt-inputBox")));
    ...
    protected void checkStepThreeData(int resultCnt) {
        ...
        try {
            List<WebElement> resultsList = browser.findElements(By.className(PropUtils.get("tag.wise-result-treeItem")));
            Assert.assertTrue("Result list should have " + resultCnt + " entries but found : " + resultsList.size(),
                    (resultCnt == resultsList.size()));
    ...
    protected void gotoStepThree() {
        try {
            WebElement nextButton = browser.findElement(By.className(PropUtils.get("tag.wise-gwt-Button-next")));
            Assert.assertNotNull("Next button was not found on page " + browser.getCurrentUrl(), nextButton);
            Assert.assertTrue("Next button should be enabled but is not.", nextButton.isEnabled());
    ...
    protected void gotoSecurityStepThree() {
        ...
        try {
            WebElement usernameInputBox = browser.findElement(By.className(PropUtils.get("tag.wise-credential-username")));
        ...
        try {
            WebElement passwordInputBox = browser.findElement(By.className(PropUtils.get("tag.wise-credential-password")));
        ...
        try {
            ...
            WebElement sendButton = browser.findElement(By.className(PropUtils.get("tag.wise-credential-send-button")));
            Assert.assertNotNull("Login button was not found in dialog " + browser.getCurrentUrl(), sendButton);
    ...
    protected void confirmPageLoaded(String pageTag) {
        ...
        Assert.assertTrue("Endpoint Config page not found. URL: " + currentUrl, currentUrl.endsWith(pageTag));
    ...
    protected void checkMessageDisclosurePanel(String msgText) {
        ...
        try {
            WebElement msgResultPanel = browser
                    .findElement(By.className(PropUtils.get("tag.wise-msg-preview.DisclosurePanel")));
            ...
            Assert.assertTrue(
                    "Expected to find [ " + msgText + " ] containted in msg but msg was [ " + msgResultContent.getText() + " ]",
                    msgResultContent.getText().contains(msgText));
    ...
    protected WebElement treeItemlookup(String key) {
        ...
        List<WebElement> treeItemsList = browser.findElements(By.className(PropUtils.get("tag.tree.item")));
    ...
    protected void confirmDialogDisplay() {
        try {
            ...
            WebElement authDialog = browser.findElement(By.className(PropUtils.get("tag.wise-authentication-dialog")));
            Assert.assertNotNull("Authenitcation Dialog was not found on page " + browser.getCurrentUrl(), authDialog);
    ...
    protected void confirmWindowAlertDisplay() {
        ...
        try {
            ...
            Assert.assertFalse("Selenium alert issue: " + e.getMessage(), e.getMessage().startsWith("WARNING:"));
        }
